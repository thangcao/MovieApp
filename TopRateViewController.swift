...
class TopRateViewController: BaseViewLoadDataController {
...
extension TopRateViewController : UITableViewDataSource, UITableViewDelegate{
...
extension TopRateViewController : UISearchBarDelegate , UISearchDisplayDelegate{
...
// Custom function
extension TopRateViewController {
    func initSearchBar() {
        searchBars.showsCancelButton = false
    ...
    func loadData() {
        if (!HandleUtil.isConnectToNetwork()){
            ...
            loadData(MOVIE_TOP_RATED, tableView: self.tableView, refreshControl: refreshControl)
        }
    }
